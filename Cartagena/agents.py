...
class personas(Agent):
        ...
        def __init__(self, n_id, modelo,poblacion,m, n_estado, t_contagio, densidad, acumedad,sexo, edades, transporte, Casillas_zona, 
            acummovilidad, movilidad, dias_cuarentena,long_paso,porcentaje_en_cuarentena):
            super().__init__(n_id,modelo)
            
            #Se definen los parámetros de la infección
            self.estado = n_estado
            # Tipos de estados:
            # Estado 1: Susceptible
            # Estado 2: Infectado leve
            # Estado 3: Infectado grave
            # Estado 4: Infectado crítico
            # Estado 5: Recuperado
        ...
        def set_sintomas(self): #Método que asigna la probabilidad de ser sintomático y asintomático
            ...
            if numsint <= 0.3: ################### siempre es esta probabilidad? ###############
        ...
        def set_edad(self, acumedad, edades): #Método que asigna la edad
            ...
            for i in range(len(lista)): ################ Preguntar esta linea ###############
                n1 = 5*(i)
                n2 = 5*(i+1)-1
                l =[n1,n2]
                rangos.append(l)
        ...
        def step(self):
            ...
            n_posx =int( self.pos[0] + round(random.uniform(-5,5))) #### En zona siempre se mueve máximo 5
